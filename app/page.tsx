  ...
  useEffect(() => {
    const detectLocalAccess = async () => {
      try {
        ...
        const timeout = setTimeout(() => socket.close(), 3000); // 3秒超时
  ...
  useEffect(() => {
    ...
    const connect = async () => {
      ...
      newSocket.onerror = (error: Event) => {
        const errorMessage = (error as ErrorEvent).message || '未知错误';
